...
def spesa_detail(spesa_id):
    ...
    return render_template('spesa_detail.html', spesa=spesa, pagamenti=pagamenti, totale_pagato=sum(p['importo_pagato'] for p in pagamenti), rimanente_da_pagare=rimanente_da_pagare, user=get_user_by_id(session.get('user_id')))
...
def add_pagamento(spesa_id):
    ...
    db = get_db()
    db.execute("INSERT INTO pagamenti (spesa_id, importo_pagato, data_pagamento, note) VALUES (?, ?, ?, ?)", (spesa_id, importo, data_pagamento, note))
    db.commit()
...
def api_scadenze():
    ...
    for scadenza in scadenze_rows:
        event = {
            'title': scadenza['descrizione'],
            'start': scadenza['data_scadenza'],
            'allDay': True
        }
        if scadenza['stato'] == 'Completato':
            event['backgroundColor'] = '#28a745'
            event['borderColor'] = '#28a745'
        if scadenza['spesa_associata_id']:
            event['url'] = url_for('spesa_detail', spesa_id=scadenza['spesa_associata_id'])
        events.append(event)
...
def edit_spesa(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            categories = [row['name'] for row in db.execute("SELECT name FROM categories ORDER BY name").fetchall()]
            return render_template('edit.html', spesa=spesa, categories=categories, user=get_user_by_id(session.get('user_id')))

        db.execute(
            "UPDATE spese SET descrizione = ?, importo = ?, categoria = ? WHERE id = ?",
